...
export default class InputAngle extends React.Component<Props, State> {
	...
	componentDidMount() {
		...
		let dragAngle: number = 0;
		let combinedAngle: number = 0;
		let angle: number;
		let rotation: number
		...
		this.gesture.on('drag', ({current, multiplier}: Gesture.DragEvent) => {
			...
			combinedAngle = angle + (previousAngle - dragAngle)
			let newValue = rotation * 360 + combinedAngle
		...
		this.gesture.on('start-drag', (origin: Point) => {
			const center = getDOMCenter(this.$sliderWrapper)
			offset.x = -(origin.y - center.y)
			offset.y =  (origin.x - center.x)
			const angleRad = Math.atan2(offset.y, offset.x)
			dragAngle = angleRad / Math.PI * 180
		...
		this.gesture.on('end-drag', () => {
			previousAngle = combinedAngle % 360
